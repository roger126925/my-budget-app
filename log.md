...
### 改善項目分析與排優先度
- 盤點現有功能缺口，新增 6 個改善項目加入 CLAUDE.md 待開發清單
- 新增項目：刪除交易 undo、轉帳備註、日期快速選昨天、週期性消費跳過某月、分期完成提示、年度統計分類趨勢
- 排優先度：P1 修壞掉的功能 → P2 高頻 UX 小改 → P3 功能補強 → P4 複雜多人協作

### Bug 修復 / 功能補強（P1）

#### 分類管理 EMOJI 可點擊
- 問題：常用 EMOJI 只是純文字顯示，無法點擊
- 修法：改為 `button` 列，點擊即填入 icon 欄；選中者顯示紫色背景高亮
- 範圍：新增分類表單 + 編輯分類表單均新增

#### 週期性消費編輯功能
- 問題：只能刪除後重建，無法修改已設定的週期性消費
- 修法：新增 `editRecurringId` / `editRecurringForm` state；列表每筆加 ✎ 按鈕，點後展開 inline 編輯表單（同新增表單欄位），儲存呼叫 `recurrings.update`

#### 刪除交易 undo 機制
- 問題：誤刪交易只能重新輸入，且 `confirm()` 彈窗打斷操作流程
- 修法：移除 `confirm()`，刪除後顯示底部黑色 toast「已刪除『備註』復原」5 秒；點「復原」重新 insert 交易並回復帳戶餘額；`undoTimer` ref 管理計時，5 秒後自動關閉

#### 快速記帳範本 UI 重構
- 問題：主頁 chips 排列佔空間，且管理（刪除）與使用混在一起
- 修法：
